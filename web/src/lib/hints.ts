...
export const HINTS = {
  ...
  operatorColours: 'Fixed colour table: every operator with 16 or more active sites has its own colour; the eight largest use a colour-blind-checked palette, the rest an extended sequence. Colours never change between releases; new entrants are appended after each data refresh. All other operators are grey. An unlisted operator you select in the dropdown borrows the first hue not used by a selected table operator for as long as it is selected. A site with several licence holders takes the colour of the first table entry it matches. The same colours are used for lines and dots in the lice charts; this site is navy and "all farms" is dark grey.',
