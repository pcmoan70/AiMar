...
export const HINTS = {
  ...
  operatorColours: 'Fixed colour table: the eight largest operators by active sites each own one hue of a colour-blind-checked palette, in a fixed order that does not change between releases. All other operators are grey. An unlisted operator you select in the dropdown borrows the first hue not used by a selected table operator for as long as it is selected. A site with several licence holders takes the colour of the first table entry it matches. The same colours are used for lines and dots in the lice charts; this site is navy and "all farms" is dark grey.',
