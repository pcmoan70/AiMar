...
export const HINTS = {
  ...
  liceChart: `Weekly average adult female lice per fish, from 2012 when weekly reporting started; the buttons switch between the last four years and the full history. Red line = 0.5 limit; grey bands = weeks reported as fallow; ▲ mechanical removal, ◆ medicinal treatment. Hover the chart for values. ${BW}`,
  liceChartOperators: `Blue line: this site's weekly adult female lice per fish. Each coloured line is the lice level at one selected operator's other farms within 150 km, seen from this site: the farms' reported values are averaged with weights 1/d² (d = great-circle distance, floor 0.5 km), a passive radial-spread assumption that ignores currents, wind and waves. Only farms that reported that week count, so fallow or silent farms never dilute the line; weeks with no reporting farm are gaps. ${BW}`,
