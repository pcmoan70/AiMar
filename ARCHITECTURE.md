...
| `components/ClimatologyLayer.tsx` | Paints a monthly climatology PNG into an image source, keeps the decoded grid for hover, and re-samples the direction arrows as the map moves (`lib/climatology.ts`) |
| `components/ClimMonthControl.tsx` | Month selector on the map, shown while a monthly-normal layer is on |
