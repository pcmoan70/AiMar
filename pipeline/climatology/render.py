...
Each PNG encodes one month of one field (and depth for currents):
    R = mean index       (0 = no data, 1..255 -> 0..max)
...
LAYER = {
    "waves": dict(max=8.0, pixel=4000, source_spacing=3000, units="m", title="Significant wave height"),
    "wind": dict(max=30.0, pixel=4000, source_spacing=3000, units="m/s", title="Wind speed (10 m)"),
    "currents": dict(max=1.0, pixel=2000, source_spacing=800, units="m/s", title="Current speed"),
}
...
def encode(mean, p90, direction, steadiness, vmax: float) -> np.ndarray:
    ok = np.isfinite(mean) & np.isfinite(direction)
    scale = lambda v: (1 + np.round(np.clip(v / vmax, 0, 1) * 254)).astype(np.uint8)
...
def render_file(path: Path, out_dir: Path, manifest: dict) -> None:
    ...
    for di, depth in enumerate(depths):
        ...
        lm = manifest["layers"].setdefault(field, {
            "title": cfg["title"], "units": cfg["units"], "max": cfg["max"], "calm": float(ds.attrs.get("calm", 0) or 0),
...
def main(argv: list[str] | None = None) -> None:
    ...
    manifest = {
        ...
        "encoding": {"R": "mean index 1..255 -> 0..max (0 = no data)", "G": "p90 index", "B": "direction to, 0..255 -> 0..360°", "A": "steadiness 1..255 -> 0..1"},
        "layers": {},
    }
